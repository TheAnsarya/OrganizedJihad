...
122. #327 Hotfix: standalone release bundle payloads for v0.2.1 installer
- Status: Completed on `feature/204-architecture-modernization`.
- Outcome target: fix installer failure when running downloaded release bundle outside source repository checkout.
- Progress update: updated `Install-OrganizedJihad.ps1` to auto-detect source-vs-bundle mode and consume bundled API/Desktop/userscript payloads, made health-check script optional in bundle mode, removed forced `-AllowNonAdmin` from GUI installer args, expanded `Publish-ReleaseArtifacts.ps1` to package `bundled/api`, `bundled/desktop-app`, `organized-jihad.user.js`, and `install-health-check.mjs`, then rebuilt and re-uploaded release assets to `v0.2.1` with `gh release upload --clobber`.
