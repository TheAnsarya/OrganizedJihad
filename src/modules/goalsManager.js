...
class GoalsManager {
     ...
     */
    findGoal(id) {
        return this.goals.shortTerm.find(g => g.id === id) || 
               this.goals.longTerm.find(g => g.id === id);
    }
    ...
    getOverdueGoals() {
        const now = Date.now();
        return {
            shortTerm: this.goals.shortTerm.filter(g => 
                g.status === 'active' && g.deadline && g.deadline < now
            ),
            longTerm: this.goals.longTerm.filter(g => 
                g.status === 'active' && g.deadline && g.deadline < now
            )
        };
    }
